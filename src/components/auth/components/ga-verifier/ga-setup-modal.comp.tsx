...
import { useSetupGA } from "../../controllers/use-setup-ga";
...
const Content = ({ secretKey, onCopy, qrCodeUrl, onClose, onNext }: ContentProps) => {
  return (
    <Stack spacing={2} px={2}>
      ...
      <ListItem variant="caption">
        <b>Notice:</b> Please save the <b>QR Code</b> or the <b>Secret Key</b> since they will help
        you setting up the Google Authenticator again in case you lose your Google Authenticator
...
export const GASetupModal = ({ open, onClose, onNext }: Props) => {
  const { data, isLoading } = useSetupGA();
  const { secretKey, qrCodeUrl } = data || {};
