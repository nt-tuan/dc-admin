...
export const ProductTemplateReview = memo(({ data = sample }) => {
  const productName = useMemo(
    () => data.vitalInformation.find((item) => item.key === "productName")?.value,
    [data]
  );
  const productDetails = useMemo(
    () => [
      ...data.vitalInformation.filter(
        (item) => !["aheccCode", "aheccDescription"].includes(item.key)
      ),
      ...data.customVital
    ],
    [data]
    ...
    <Fragment>
      <div className="row">
        <div className="col-xl-3 col-12 text-center mb-4">
          ...
          <img
            src={data.details.productImage.url}
            alt="Product"
            className="mt-2"
            style={{ width: "100%" }}
          />
